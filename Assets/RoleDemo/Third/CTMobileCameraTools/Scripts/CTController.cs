    ...
    public class CTController : MonoBehaviour
    {
        ...
        private void Zoom()
        {
            if (Input.touchCount == 2 && Input.GetTouch(0).phase == TouchPhase.Moved && Input.GetTouch(1).phase == TouchPhase.Moved)
            {
                ...
                if (Mathf.Abs(Input.GetTouch(0).deltaPosition.x) < Mathf.Abs(Input.GetTouch(0).deltaPosition.y) && Mathf.Abs(Input.GetTouch(1).deltaPosition.x) < Mathf.Abs(Input.GetTouch(1).deltaPosition.y))
                {
                    return;
                }
                ...
                if (Input.GetTouch(0).deltaPosition.x * Input.GetTouch(1).deltaPosition.x < 0 && canControl)
                {
                    ...
                    //GET DELTA MOVEMENT
                    float delta = screenScaler * Mathf.Max(Mathf.Abs(Input.GetTouch(0).deltaPosition.x), Mathf.Abs(Input.GetTouch(1).deltaPosition.x)) * GetTimeSinceLastFrame();

                    //SET SIGN (ZOOM IN OR ZOOM OUT)
                    if ((Input.GetTouch(0).deltaPosition.x > 0 && Input.GetTouch(0).position.x < Screen.width / 2) || (Input.GetTouch(0).deltaPosition.x < 0 && Input.GetTouch(0).position.x > Screen.width / 2))
                        delta *= -1;
            ...
            if (Mathf.Abs(accumulatedZoom) > 0.01f)
            {
                ...
                {
                    if ((-zoomNode.transform.localPosition.z < zoomMinHeight && accumulatedZoom > 0) || (-zoomNode.transform.localPosition.z > zoomMaxHeight && accumulatedZoom < 0))
                    {
                        accumulatedZoom = 0;
                    }
